...
var address = "tNULSeBaMu38g1vnJsSZUCwTDU9GsE5TVNUtpD";
var remark = 'create tradingOrder....';
var txType = 29;
var fee = 100000; //手续费

var defaultAsset = { //本链默认资产，用于生成手续费
    assetsChainId: 2,
    assetsId: 1
};
...
async function tradingOrder(tradingOrderInfo) {
    var inOrOutputs = await createCoinData(tradingOrderInfo);
...
async function createCoinData(tradingOrderInfo) {
    var balanceInfo = await getBalance(defaultAsset.assetsChainId, tradingOrderInfo.assetsChainId, tradingOrderInfo.assetsId, tradingOrderInfo.address);
    var inputs = [],
        outputs = [];
    var input = {
    ...
    if (tradingOrderInfo.assetsChainId === defaultAsset.assetsChainId && tradingOrderInfo.assetsId === defaultAsset.assetsId) {
        ...
        //如果不是要额外收取手续费
        inputs.push(input);
        var _balanceInfo = await getBalance(defaultAsset.assetsChainId, defaultAsset.assetsChainId, defaultAsset.assetsId, tradingOrderInfo.address);
        inputs.push({
            address: tradingOrderInfo.address,
            assetsChainId: tradingOrderInfo.assetsChainId,
            assetsId: tradingOrderInfo.assetsId,
